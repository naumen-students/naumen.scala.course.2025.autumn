...
object Breakfast extends ZIOAppDefault {
    ...
    def makeBreakfast(eggsFiringTime: Duration,
        ...
        val eggsTask = ZIO.sleep(eggsFiringTime) *>
          ZIO.succeed(LocalDateTime.now()).map(t => Map("eggs" -> t))

        val saladTask = for {
            _ <- ZIO.sleep(saladInfoTime.cucumberTime)
            _ <- ZIO.sleep(saladInfoTime.tomatoTime)
            t <- ZIO.succeed(LocalDateTime.now())
        } yield Map("saladWithSourCream" -> t)

        val waterAndTeaTask = for {
        ...
        ZIO.collectAllPar(List(eggsTask, saladTask, waterAndTeaTask))
