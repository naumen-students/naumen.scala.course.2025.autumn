...
object Test extends TestSuite {
    ...
    val tests = Tests{
        ...
        val v1 = Exercises.Vector2D(1.0, 0.0)
        val v2 = Exercises.Vector2D(0.0, 1.0)
        val v3 = Exercises.Vector2D(1.0, 1.0)
        val v4 = Exercises.Vector2D(2.0, 2.0)
        'test_sumScalars - {
            val expected = 4.0
            val actual = Exercises.sumScalars(v1, v2, v3, v4)
            assert(Math.abs(actual - expected) < 0.0001)
        }
        'test_sumCosines - {
            val expected = 1.0
            val actual = Exercises.sumCosines(v1, v2, v3, v4)
            assert(Math.abs(actual - expected) < 0.0001)
        }
        ...
        'test_sortByHeavyweight - {
            ...
            assert(Exercises.sortByHeavyweight() == expectedOrder)
        }
    }
}
