import utest._

object Test extends TestSuite{

    val balls: Map[String, (Int, Double)] =
        Map(
            "Aluminum" -> (3,   2.6889), "Tungsten" ->  (2,   19.35), "Graphite" ->  (12,  2.1),   "Iron" ->      (3,   7.874),
        )

    val balls2: Map[String, (Int, Double)] =
        Map(
            "Aluminum" -> (2,   2.6889), "Tungsten" ->  (12,   19.35), "Graphite" ->  (1,  2.1),   "Iron" ->      (2,   7.874),
        )

    val tests = Tests{
        'test_divBy3Or7 - {
            assert(Exercises.divBy3Or7(1, 3) == Seq(3))
            assert(Exercises.divBy3Or7(5, 9) == Seq(6, 7, 9))
            assert(Exercises.divBy3Or7(0, 100) == Seq(0, 3, 6, 7, 9, 12, 14, 15, 18, 21, 24, 27, 28, 30, 33, 35, 36, 39, 42, 45, 48, 49, 51, 54, 56, 57, 60, 63, 66, 69, 70, 72, 75, 77, 78, 81, 84, 87, 90, 91, 93, 96, 98, 99))
        }
        'test_sumOfDivBy3Or5 - {
            assert(Exercises.sumOfDivBy3Or5(1, 5) == 8L)
            assert(Exercises.sumOfDivBy3Or5(5, 9) == 20L)
            assert(Exercises.sumOfDivBy3Or5(5, 15) == 57L)
        }
        'test_primeFactor - {
            assert(Exercises.primeFactor(80) == Seq(2, 5))
            assert(Exercises.primeFactor(98) == Seq(2, 7))
            assert(Exercises.primeFactor(30) == Seq(2, 3, 5))
        }
        'test_sumByFunc - {
            assert(Exercises.sumScalars(
                Exercises.Vector2D(1,2),
                Exercises.Vector2D(3,4),
                Exercises.Vector2D(5,6),
                Exercises.Vector2D(7,8)) == 94.0)
            assert(Exercises.sumCosines(
                Exercises.Vector2D(3,4),
                Exercises.Vector2D(4,3),
                Exercises.Vector2D(5,12),
                Exercises.Vector2D(-12,5)) == 0.96)
        }
        'test_sortByHeavyweight - {
            assert(Exercises.sortByHeavyweight(balls) ==  List("Aluminum", "Tungsten", "Iron", "Graphite"))
            assert(Exercises.sortByHeavyweight(balls2) ==  List("Graphite", "Aluminum", "Iron", "Tungsten"))
        }
    }
}
