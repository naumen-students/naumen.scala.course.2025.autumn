...
object Exercises {
  ...
  def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = {
    IntStream.rangeClosed(iFrom, iTo)
  ...
  def primeFactor(number: Int): Seq[Int] = {
    val set: mutable.MutableList[Int] = new mutable.MutableList[Int]
    var temp = number
    if (temp % 2 == 0) {
      temp = completeDivide(temp, 2)
      set += 2
    }
    var div: Int = 3
    while (div <= temp) {
      if (temp % div == 0) {
        temp = completeDivide(temp, div)
        set += div
      }
      div = div + 2
    }
  ...
  @tailrec
  private def completeDivide(num: Int, div: Int): Int = {
    if (num % div == 0) completeDivide(num / div, div) else num
  }
  ...
  */
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
    ballsArray
      .map({
        case (key, (intVal, doubleVal)) => (key, 4 * intVal * intVal * intVal * doubleVal * java.lang.Math.PI / 3)
      })
      .toSeq
      .sortBy(_._2)
      .map(_._1)
