...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        @tailrec
        def findFactors(n: Int, divisor: Int, factors: Seq[Int]): Seq[Int] = {
            if (n == 1) {
                factors.distinct.sorted
            } else if (n % divisor == 0) {
                findFactors(n / divisor, divisor, factors :+ divisor)
            } else {
                findFactors(n, divisor + 1, factors)
            }
        }

        if (number <= 1) {
            Seq.empty[Int] // Возвращаем пустой список
        } else {
            findFactors(number, 2, Seq.empty[Int])
        }
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        // Вычисление объема шарика: (4/3) * Pi * r^3
        def volume(radius: Int): Double = (4.0 / 3.0) * java.lang.Math.PI * math.pow(radius, 3)

        // Вычисление массы шарика: плотность * объем
        def mass(radius: Int, density: Double): Double = density * volume(radius)

        // Преобразование Map в List[(String, Double)] где Double - масса шарика
        val massList = ballsArray.map { case (name, (radius, density)) =>
            (name, mass(radius, density))
        }.toList

        // Сортировка списка по массе и извлечение названий материалов
        massList.sortBy(_._2).map(_._1)
    }
}
