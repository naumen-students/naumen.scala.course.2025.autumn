object Exercises {
    ...
    def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, funcName: (Vector2D, Vector2D) => Double, rightVec0: Vector2D, rightVec1: Vector2D):Double = {
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.toSeq.map {
            case (name, (radius, density)) => (name, (4 / 3d * Math.PI * Math.pow(radius, 3) * density))
