object Exercises {
  ...
  def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = {
    ...
    sum // Возвращаем результат (последнее выражение в функции автоматически возвращается)
  }
  ...
  def primeFactor(number: Int): Seq[Int] = {
    if (number < 2) return Seq()

    var n = number
    var divisor = 2
    var factors = scala.collection.mutable.Set.empty[Int]
  ...
  /*
    ...
    */
  // Добавляем параметр func - функция, которая будет применяться к парам векторов
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
    ballsArray.toSeq
      ...
      .sortBy { case (name, mass) => mass }
      .map { case (name, mass) => name }
  }
}