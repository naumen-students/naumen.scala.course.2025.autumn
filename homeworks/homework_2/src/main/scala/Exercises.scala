...
object Exercises {
    ...
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = {
      Range.inclusive(iFrom, iTo).filter(i => i % 3 ==0 || i % 5 == 0).sum
    }
    ...
    Число 80 раскл.s  адывается на множители 1 * 2 * 2 * 2 * 2 * 5, результат выполнения функции => Seq(2, 5).
    Число 98 можно разложить на множители 1 * 2 * 7 * 7, результат выполнения функции => Seq(2, 7).*/
    /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
    def primeFactor(number: Int): Seq[Int] = {
      @tailrec
      def factorizer (div: Int, num: Int, acc: List[Int]): List[Int] = {
        if (num == 0) acc
        else if (div * div > num) acc :+ num
        else if (num % div == 0) factorizer(div, num / div, acc :+ div)
        else factorizer(div + 1, num, acc)
      }
      factorizer(2, number, List()).seq.distinct
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
      ballsArray.toSeq.sortBy {
        case (_, (radius, density)) => java.lang.Math.PI * radius * radius * radius * 4.0 / 3.0 * density
      } map {
        case (name, (_, _)) => name
      }
    }
