...
object Exercises {
  ...
  def primeFactor(number: Int): Seq[Int] = {
    @tailrec
    def factors(num: Int, divider: Int, set: Set[Int]): Set[Int] = {
      if (num == 1) set
      else if (num % divider == 0) factors(num / divider, 2, set + divider)
      else factors(num, divider + 1, set)
    }

    if (number <= 1) Seq.empty
    else factors(number, 2, Set.empty).toSeq.sorted
  }
  ...
  */
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
    def calculateMass(radius: Int, density: Double): Double = {
      val volume = (4.0 / 3.0) * java.lang.Math.PI * java.lang.Math.pow(radius, 3)
      density * volume
    }
    ...
    ballsArray
      ...
      .toSeq
      .sortBy(_._2)  // Сортировка по массе (второй элемент кортежа)
      .map(_._1)     // Извлечение только имен
  }
}