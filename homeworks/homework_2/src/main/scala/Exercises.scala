...
object Exercises {
    ...
    {
        primeFactorRecursive(number, 2, Seq.empty)
    }

    @tailrec
    def primeFactorRecursive(number: Int, divisor: Int, divisors: Seq[Int]): Seq[Int] =
    {
        if(number <= 1)
        {
            divisors.distinct
        ...
        {
            primeFactorRecursive(number / divisor, divisor, divisors.appended(divisor))
        }
        else
        {
            primeFactorRecursive(number, divisor + 1, divisors)
        }
    }
    ...
    */
    ...
    {
        val namesWithMass = for((name, (radius_cm, density_g_to_cm3)) <- ballsArray.toSeq) yield
        {
            val volume_cm3 = calculateVolume(radius_cm)
            val mass_g = calculateMass(density_g_to_cm3, volume_cm3)
            (name, mass_g)
        }

        val namesWithMassSorted = namesWithMass.sortBy(nameWithMassTuple => nameWithMassTuple._2)

        for((name, mass_g) <- namesWithMassSorted) yield name
    }

    def calculateVolume(radius_cm: Int): Double =
    {
        (4.0 / 3.0) * PI * math.pow(radius_cm.toDouble, 3.0)
    }

    def calculateMass(density_g_to_cm3: Double, volume_cm3: Double): Double =
    {
        density_g_to_cm3 * volume_cm3
    }
}