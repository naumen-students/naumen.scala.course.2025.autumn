object Exercises {
  ...
  def primeFactor(number: Int): Seq[Int] = {
    ...
    while (d * d <= abs_number) {
      while (abs_number % d == 0) {
        factors += d
        abs_number /= d
      }

      d = if (d == 2) d + 1 else d + 2
    }
  ...
  def sumByFunc(
      leftVec0: Vector2D,
      leftVec1: Vector2D,
      reduce: (Vector2D, Vector2D) => Double,
      rightVec0: Vector2D,
      rightVec1: Vector2D
  ) = {
    reduce(leftVec0, leftVec1) + reduce(rightVec0, rightVec1)
  }
  ...
  def sortByHeavyWeight(
    ...
    ballsArray.toList
      .sortBy { case (name, (radius, density)) =>
        4 / 3 * Math.pow(radius, 3) * java.lang.Math.PI * density
      }
      .map(_._1)
  }
}
