...
object Exercises {
  ...
  def primeFactor(number: Int): Seq[Int] = {
    @tailrec
    def primeFactorRec(number: Int, divisor: Int, acc: Seq[Int]): Seq[Int] = {
      if (divisor == number)
        return acc.appended(divisor)

      if (number % divisor == 0)
        primeFactorRec(number / divisor, divisor, acc.appended(divisor))
      else
        primeFactorRec(number, divisor + 1, acc)
    }

    if (number < 2)
      return Seq[Int]()

    primeFactorRec(number, 2, Seq[Int]()).distinct
  }
  ...
  def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D,
    ...
    func.apply(leftVec0, leftVec1) + func.apply(rightVec0, rightVec1)
  }
  ...
  */
  ...
  private def volume(radius: Int): Double =  4/3 * Math.PI * radius * radius * radius

  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
    ballsArray.iterator.map(
      it => (it._1, volume(it._2._1) * it._2._2)
    ).toSeq.sortBy(_._2).map(_._1).toSeq
  }
}
