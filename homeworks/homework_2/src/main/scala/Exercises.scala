object Exercises {
    ...
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = {
      (for {i <- iFrom to iTo
           if i % 3 == 0 || i % 5 == 0} yield i.toLong).sum
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        def mass(radius: Int, density: Double): Double = {
            val r3 = radius.toDouble * radius * radius
            (4.0 / 3.0) * java.lang.Math.PI * r3 * density
        }
        val nameAndMass =
            for ((name, (radius, density)) <- ballsArray.toSeq)
                yield (name, mass(radius, density))

        val sorted = nameAndMass.sortBy(_._2)
        sorted.map(_._1)
    }
